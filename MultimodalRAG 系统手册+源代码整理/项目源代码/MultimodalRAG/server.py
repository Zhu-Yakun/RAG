...
if not static_dir.is_dir():
    ...
    try:
        ...
        print(f"[INFO] Mounted static directory: {static_dir.resolve()}")
...
if images_dir.is_dir():
    try:
        ...
        print(f"[INFO] Mounted images directory: {images_dir.resolve()}")
    except Exception as e:
        print(f"ERROR mounting images directory '{images_dir}': {e}")
else:
    print(f"WARNING: Images directory '{images_dir}' not found. Image preview will fail!")

# --- 新增：图片请求日志中间件 ---
class ImageRequestLoggerMiddleware(BaseHTTPMiddleware):
    async def dispatch(self, request, call_next):
        if request.url.path.startswith("/images/"):
            print(f"[IMAGE REQUEST] {request.method} {request.url.path}")
        response = await call_next(request)
        if request.url.path.startswith("/images/"):
            if response.status_code != 200:
                print(f"[IMAGE ERROR] {request.url.path} -> {response.status_code}")
        return response
app.add_middleware(ImageRequestLoggerMiddleware)
...
async def handle_query(
    ...
    # --- 新增：修正image_path字段为文件名 ---
    for doc in retrieved_docs:
        if doc.get('image_path'):
            # import os
            doc['image_path'] = os.path.basename(str(doc['image_path']))
    final_response_data = {
